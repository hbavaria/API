...
export class WebSocketHandler {
   ...
   * Constructor.
   */
  constructor(private _authCookie: string) {
    this.initPromise();
  ...
  initializeWebSocket(): Promise<any> {
    const req = new Promise<any>(async (resolve, reject) => {
      ...
      const res = await fetch(
        `${process.env.OE_WEB_SERVER_URL}/api/Account/OneTimeToken`,
        {
          method: "GET",
          headers: {
      ...
      if (res.ok) {
        await res.text().then(token => {
          let oeWebUrl = new URL(`${process.env.OE_WEB_SERVER_URL}`);
          let wsUrl = `ws://${oeWebUrl.host}/api/ws`;
